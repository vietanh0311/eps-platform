...
npm run sync:ambassador                                  # đồng bộ Campaign từ Ambassador (public API, không cần .env)
```
...
**Chi phí video bắt buộc (2026-07-23):** `Video.productionCost` là `Int?` — `NULL` = chưa điền,
`0` = xác nhận thật sự miễn phí. Ô chi phí ở `/videos/new` và trang sửa video là bắt buộc, không
còn fallback ngầm về `Talent.productionFeePerVideo`. `/videos` có filter "Chưa có chi phí" +
chọn nhiều dòng điền nhanh 1 giá chung (`bulkSetProductionCost`). Video thuộc kỳ lương đã
`APPROVED`/`PAID` bị khóa sửa `productionCost`/`airDate`/`campaignId` với MM (Team Tech/Team
Finance luôn sửa được) — CFO/Tech bấm "Mở lại kỳ lương" (`/payroll/[id]`) để mở khóa tạm, tính
nháp lại thủ công sau khi sửa xong.
...
## Đồng bộ Campaign Ambassador (bổ sung module 2)

Gộp thẳng campaign từ `ambassador.koc.com.vn/chien-dich` vào bảng `campaigns` hiện có (không bảng
riêng) — `src/server/ambassador/client.ts` + `sync.ts`. Khác ScaleF: 2 endpoint **public, không
cần đăng nhập/token** — `GET /api/public/news?type=home_list` (danh sách campaign) và
`GET /api/public/partners` (suy tên nhãn hàng đẹp từ slug URL).

1. `npm run sync:ambassador` (hoặc nút "Đồng bộ Ambassador ngay" trên `/campaigns`) — upsert theo
   `externalKey = "ambassador:<_id>"`, chỉ ghi đè cột Ambassador làm chủ
   (`descHtml`/`sourceUrl`/`coverUrl`/`startDate`/`endDate`/`lastSyncedAt`/`raw`) — không bao giờ
   đụng `name`/`brief`/`mmId`/`status`/`contractValue`/`notes` (người dùng tự sửa tay). Không xóa
   campaign nào — hết hạn tự nhiên biến mất khỏi feed, không phải bị xóa.
2. Campaign mới đồng bộ về có `mmId = null` ("chưa nhận") — MM tự bấm "Nhận campaign này" ở trang
   chi tiết (set `mmId = mình`), hoặc Team Tech/Team Finance chọn MM bất kỳ. `/campaigns` có chip
   "Của tôi" / "Chưa nhận" / "Tất cả" + badge nguồn để phân biệt với campaign tạo tay.
3. `desc_html` (mô tả từ Ambassador) hiển thị dạng văn xuôi thường (`stripHtml`,
   `src/lib/labels.ts`) — **không** `dangerouslySetInnerHTML`, vì đây là dữ liệu ngoài (rủi ro XSS
   thật). Xem thể lệ đầy đủ qua link `source_url` (mở tab mới).
4. Khi lên VPS/Coolify: Coolify Scheduled Task chạy `npm run sync:ambassador`, 1 lần/ngày — không
   cần credentials gì (khác ScaleF).
